...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--minimal-output",
        action="store_true",
        help="Only print the absolute minimum: errors and action triggers. Suppresses normal recognition prints.",
    )
...
def trigger_action(container_id: str, action_name: str, dry_run: bool = False) -> None:
    ...
    try:
        ...
        print(f"ERROR: Timed out while triggering action '{action_name}'.", file=sys.stderr, flush=True)
...
def main() -> int:
    ...
    try:
        while True:
            ...
            # During cooldown, skip expensive snapshot fetch + inference work.
            # This keeps CPU load much lower after a response fires.
            if (now - last_trigger_time) < args.cooldown_sec:
                if args.show:
                    cooldown_remaining = max(0.0, args.cooldown_sec - (now - last_trigger_time))
                    frame_bgr = np.zeros((240, 640, 3), dtype=np.uint8)
                    cv2.putText(
                        frame_bgr,
                    ...
                    if cv2.waitKey(1) & 0xFF == 27:
                        break
                time.sleep(min(0.1, args.cooldown_sec))
                continue
