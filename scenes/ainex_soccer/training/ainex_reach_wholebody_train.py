"""
Train PPO for AINex whole-body reach: robot walks around the table to get closer
to the ball, then reaches with the arm.
"""
import os
import argparse
import csv
...
def main():
    parser = argparse.ArgumentParser(description="Train PPO for AINex whole-body reach (walk + reach).")
    ...
    print(f"Launching {num_cpu} parallel environments (whole-body reach)...")

    env = SubprocVecEnv([make_env(i, args.seed) for i in range(num_cpu)])

    log_dir = os.path.join("logs", "ainex_reach_wholebody")
    ...
    # Larger policy for whole-body (legs + arms)
    ...
    print(f"Training PPO for {args.timesteps:,} timesteps (whole-body reach)...")
